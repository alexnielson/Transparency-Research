...
* A .csv file of the Transaction Group Table, containing the following fields:
  * id
  * name
* A .csv file of the Vendor Table, containing the following fields:
  * vendor_id
  * vendor_name
  * entity_id
...
* I considered joining the data from the Transaction Table with the data from the Transaction Group and Vendor tables. Doing so would have allowed me to send the Census Bureau a .csv file that wouldn't require additional joining in order to understand the information. I decided not to do so for the following reasons:
...
create_govt_lvl_report <- function(active_govts, govt_lvl, fy_1, fy_2) {
  ...
  active_govts %>% 
    ...
    select(
      entity_name, entity_id = t_id, fiscal_year, transaction_type, posting_date,
      amount, description, vendor_id, org1, org2, org3, org3, cat1, cat2,
...
```{r}
query_vendor_table <- function(t_id) {
  # Query an entity's vendors.
  #
  # Arguments:
  #   t_id (num): The entity's Transparency ID, as queried from Salesforce
  #     rather than Transparency.
  ...
  dbGetQuery(
    odbc_aws,
    paste("
      SELECT id AS vendor_id, name AS vendor_name, entity_id 
      FROM vendor
...
report_vendors %>% 
  write_file("id_crosswalk_vendors", fy_1, fy_2)
