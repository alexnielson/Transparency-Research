...
WHERE 
  Contact.IsDeactivated__c = FALSE
AND 
  Account.RecordTypeId IN (
...
contact_info_less_than_one_million <- 
  aws_2017_entity_totals %>% 
  full_join(sf_contact_info,
            by = "transparency_id") %>% 
  filter(total_exp < 1000000 & total_rev < 1000000) %>% 
  mutate(entity_name = ifelse(!is.na(entity_name.y), 
                              entity_name.y, 
                              entity_name.x)) %>% 
  select(entity_name, govt_type, total_exp, total_rev, entity_phone, 
         contact_full_name, contact_phone, contact_email, contact_title, 
         contact_position, contact_first_name, contact_last_name)

entities_only <- 
  contact_info_less_than_one_million %>% 
  distinct(entity_name, entity_phone)
